...
# Fecha : 26/03/2026 
# Propósito: realizar un algoritmo greedy para resolver el problema de la p-mediana
##############################################################################
...
greedy_establish_servers <- function(nodes, p, limit_capacity){
  ...
  while (length(servers) < p && length(candidates) > 0){
    ...
    nodes[[server_idx]]$used_capacity <- potential_server$used_capacity
    servers[[length(servers) + 1]] <- potential_server
  }
  
  unassigned <- sapply(nodes, function(nd) is.null(nd$server))
  
  if (all(!unassigned)) {
    return(list(nodes = nodes, servers = servers))
  } else {
    stop("Greedy heuristic failed to build a complete feasible solution")
  }
}
...
objective_function <- function(solution) {
  ...
  for (server in solution$servers) {
    for (node in solution$nodes) {
      if (!is.null(node$server) && node$server == server$name) {
...
evaluate_solution_quality <- function(solution, best_value){
  data <- objective_function(solution)
  gap <- (data - best_value) / best_value * 100
  gap
}
...
solution <- greedy_establish_servers(data$nodes, data$p, data$capacity)
objective_function(solution)
evaluate_solution_quality(solution, 740)
...
solution <- greedy_establish_servers(data$nodes, data$p, data$capacity)
objective_function(solution)
evaluate_solution_quality(solution, 751)
...
solution <- greedy_establish_servers(data$nodes, data$p, data$capacity)
objective_function(solution)
evaluate_solution_quality(solution, 651)
plot_solution_connections(solution,7)
...
solution <- greedy_establish_servers(data$nodes, data$p, data$capacity)
objective_function(solution)
evaluate_solution_quality(solution, 664)
plot_solution_connections(solution,10)